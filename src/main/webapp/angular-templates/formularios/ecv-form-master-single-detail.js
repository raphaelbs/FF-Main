/**
 * [ecvFormMasterSingleDetail]
 * Diretiva representativa do formuário mestre/single detalhe.
 * Raphael Brandão - 09/01/2016
 */
angular.module('financeiro').directive('ecvFormMasterSingleDetail', ['$http', '$ecv',
	function($http, $ecv){
	return {
		restrict: 'E',
		templateUrl: GetDOMINIOEndpoint('angular-templates/formularios/ecv-form-master-single-detail.html'),
		scope: {
			data: '='
		},
		link: function(scope, elem){
			...
			scope.validar = function(form){
				...
				if(!comp.valor) return $ecv.toast.erro(
						'Botão de ação GRAVAR sem [valor].<br>Cadastre a rota do POST no campo c13_204 para este componente (id: ' + comp.id + ').', 
						'Erro no botão de GRAVAR');
				estaCarregando(comp, true);
								
				// Persiste no banco
				$http.post(GetRESTEndpoint(comp.valor), $ecv.modelo.copiarELimpar(scope.data.model))
				.then(function(res){
					estaCarregando(comp);
					if(res.data.Resultado === 'ERRO') return $ecv.toast.erro(res.data.Erro, 'Houve um erro ao inserir');
					
		        	scope.data.model = res.data.Retorno;
					scope.data.telas.forEach(function(tela){
					...
					$ecv.toast.ok('Inserido com sucesso!');
				}, function(err){
					estaCarregando(comp);
